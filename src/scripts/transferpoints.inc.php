...
$from_account_number = $_SESSION['u_id'];
...
if (isset($_SESSION['u_id'])){
	...
	if($purchase_amount <= 0){
		...
		if($purchase_amount > $sender_current_points){
			header("Location: ../transfer-points.php?log=notenoughpoints");
			exit();
		}else{
...
) VALUES ('$transfer_date', '$member_id', '$purchase_amount', '$to_account_number', '$add_info', '0')";
	
			$result = mysqli_query($conn, $sql);

			header("Location: ../transfer-points.php?log=success");
			exit();	
		}
	}
	
} else {
	header("Location: ../transfer-points.php?error=notloggedin");
	exit();
}